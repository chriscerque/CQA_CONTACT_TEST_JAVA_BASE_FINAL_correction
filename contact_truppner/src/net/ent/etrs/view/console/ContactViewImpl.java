...
import net.ent.etrs.commons.ihm.Ihm;
import net.ent.etrs.commons.ihm.IhmFactory;
...
public class ContactViewImpl implements ContactView {
    ...
    public void afficherContacts(final List<Contact> lstContacts) {
        for (int i = 0; i < lstContacts.size(); i++) {
            afficherContact(lstContacts.get(i));
        }
    }

    @Override
    public void afficherCoordonnees(final Contact contact) {
        AffichageConsole.afficherMessageAvecSautLigne(contact.getNom().concat(" ").concat(contact.getPrenom()));
    ...
    public Contact modifierContact(Contact contact)
    {
        ...
        if(typeCoordonnee.equals(TypeCoordonnee.TELEPHONE))
        {
            try {
                coordonnee = saisirCoordonneesContactTelephone(contact);
            } catch (CoordonneeException e) {
                this.ihm.afficherChaine(e.getMessage());
            }
        }
        else
        {
            try {
                coordonnee = saisirCoordonneesContactMail(contact);
            } catch (CoordonneeException e) {
                this.ihm.afficherChaine(e.getMessage());
            }
        }
        ...
        try {
            contact.ajouterCoordonnees(coordonnee);
        } catch (CoordonneeException e) {
            this.ihm.afficherChaine(e.getMessage());
        }
    ...
    public Contact saisirContact() throws ContactException {
        ...
        AffichageConsole.afficherMessageSansSautLigne("Date de naissance (jj/mm/aaaa) : ");
    ...
    /**
     * Méthode permettant de classer les coordonnées du contact entré en paramètre dans un certain ordre.
     * @param contact: Contact
     * @return List<Coordonnee>
     */
    //TODO: à faire à la fin si le temps
...
//        Collections.sort(coordonneesTriee, new TypeCoordonneeComparator());
    ...
    /**
     * Méthode permettant de saisir/sélectionner les différentes propriétés d'un Mail (Coordonnee) et de l'instancier en le retournant.
     * @param contact: Contact
     * @return Coordonnee(Mail)
     * @throws CoordonneeException: Si l'un des paramètres du Mail(Coordonnee), instancié en retour, est NULL.
     */
    private Coordonnee saisirCoordonneesContactMail(final Contact contact) throws CoordonneeException {
        ...
        return EntitiesFactory.fabriquerMail(contact, typeMail, adresseMail);
    }

    /**
     * Méthode permettant de sélectionner à partir de l'enum TypeMail un type de mail.
     * @return TypeMail
     */
    ...
    /**
     * Méthode permettant de saisir/sélectionner les différentes propriétés d'un Telephone (Coordonnee) et de l'instancier en le retournant.
     * @param contact: Contact
     * @return Coordonnee(Telephone)
     * @throws CoordonneeException: Si l'un des paramètres du Telephone(Coordonnee), instancié en retour, est NULL.
     */
    ...
    /**
     * Méthode permettant de sélectionner à partir de l'enum TypeNumero un type de numéro.
     * @return TypeNumero
     */
    ...
    /**
     * Méthode permettant de choisir dans l'enum TypeCoordonnee un type de coordonnée.
    ...
    /**
     * Méthode permettant de sélectionner une coordonnée parmis toutes les coordonnées du contact entré en paramètre.
     * @param contact: Contact
     * @return Coordonnee
     * @throws CoordonneeException: Si le contact ne possède aucune coordonnée.
     */
