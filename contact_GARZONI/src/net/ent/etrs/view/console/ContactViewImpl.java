...
public class ContactViewImpl implements ContactView {
    ...
    public void afficherContacts(final List<Contact> lstContacts) {
        ...
        for(Contact contact : lstContacts) {
            sb.append("Contact ").append(contact.getNom() + " ").append(contact.getPrenom() + " ");
            sb.append(contact.getDateNaissance() + "\n");
            sb.append("\tCoordonnées :\n");
            for(Coordonnee coordonnee : contact.getCoordonnees()) {
                sb.append("\t\t").append(coordonnee.getTypeCoordonnee().getLibelle());
                if(coordonnee instanceof Mail) {
                    sb.append("(").append(((Mail) coordonnee).getTypeMail()).append(") : ").append(((Mail) coordonnee).getAdresseMail());
                }
                else {
                    sb.append(" : ").append(((Telephone) coordonnee).getNumero()).append("\t").append(((Telephone) coordonnee).getTypeNumero());
                }
            }
        }
