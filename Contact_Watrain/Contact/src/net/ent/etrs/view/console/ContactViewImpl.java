...
import net.ent.etrs.utils.AffichageConsole;
import net.ent.etrs.utils.LectureConsole;
...
public class ContactViewImpl implements ContactView {
    ...
    @Override
    public void afficherContacts(List<Contact> lstContacts) {
        for(Contact c : classerContact(lstContacts)) {
            afficherContact(c);
        }
    }
    ...
    public void afficherCoordonnees(final Contact contact) {
        for (Coordonnee c : classerCoordonnee(contact)) {
            if (c instanceof Telephone) {
                telephoneView.afficherCoordonnee((Telephone) c);
            } else if (c instanceof Mail) {
                mailView.afficherCoordonnee((Mail) c);
            }
        }
    ...
    private List<Contact> classerContact(List<Contact> contacts) {
        List<Contact> contactsTriee = new ArrayList<>(contacts);
        contactsTriee.sort(new ContactComparator());
        return Collections.unmodifiableList(contactsTriee);
    }
